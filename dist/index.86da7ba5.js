...
if (!localStorage.getItem("arrayOfObject")) localStorage.setItem("array", JSON.stringify(arrayOfObject));
...
const deleteObject = (delId)=>{
    arrayOfObject = arrayOfObject.filter((retainId)=>{
        if (retainId.id !== delId) return retainId;
    });
    render();
};
const editObject = (EditId)=>{
    let clickedObject = arrayOfObject.find((object)=>object.id === EditId);
...
const render = ()=>{
    if (listRef) {
        let list = "";
        for (let item of arrayOfObject)list += `<div class="d-flex align-items-center justify-content-between p-2 border-bottom border-primary">
...
if (btnRef) btnRef.addEventListener("click", ()=>{
    if (nameRef.value !== "") {
        if (editId === 0) arrayOfObject.push({
            id: getRandomNumber(),
            name: nameRef.value
        });
        else {
            arrayOfObject = arrayOfObject.map((object)=>{
                if (object.id === editId) return {
                    ...object,
                    id: editId,
                    name: nameRef.value
                };
        ...
        nameRef.value = "";
        render();
    } else nameRef.classList.replace("is-invalid", "border-primary");
});
nameRef.addEventListener("keyup", ()=>{
    if (nameRef.value !== "") nameRef.classList.replace("is-invalid", "border-primary");
    else nameRef.classList.replace("border-primary", "is-invalid");
});
render();
